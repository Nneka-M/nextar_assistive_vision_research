...
if __name__ == "__main__":
    import sys
    from inference import load_model, load_image, run_detection, \
        build_scene_description, get_annotated_image
    from explainer import build_explanation_template, build_pidgin_from_scene
    ...
    print(f"\n[XAI] Running full pipeline on: {image_path}")
    print("───────────────────────────────────────────────────────")

    img_array, rgb_float = load_image(image_path)
    detections          = run_detection(yolo, image_path)
    scene               = build_scene_description(detections)
    annotated_rgb       = get_annotated_image(yolo, image_path)
    ...
    if detections:
        ...
        save_comparison_plot(
            ...
            annotated_rgb, scene, "./outputs/comparison.jpg"
        )
    else:
        region = "di whole image"
        heatmap = np.zeros(rgb_float.shape[:2])

    pidgin = build_pidgin_from_scene(scene, region)
